...
def updateToDatabase(src_id,dst_id,emp_name,fname):
    ...
    try:
        ...
        update_statement = """UPDATE [HRG].[dbo].[Emp Files Paystubs] SET [IsUploaded] = ?, [UploadedDateTime] = ? 
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print("error g is:",g)
        try:
            xpath = "//span[normalize-space()='Paylocity Historical Documents']"
            no_of_documents = driver.find_elements(By.XPATH,xpath)
            initial_count = len(no_of_documents)
            print("Before upload count:",initial_count)
        except:
            xpath = "//*[contains(@class,'table-content_row__31sbr')]"
            no_of_documents = driver.find_elements(By.XPATH,xpath)
            initial_count = len(no_of_documents)
            print("Before upload count:",initial_count)
        with open("Before upload count.csv",'a',encoding='utf-8') as f:
        ...
        for ufile in ufiles:
            ...
            while(1):
                try:
                    ...
                    actions.perform()
                    #folder
                    xpath = '//*[text() ="Please select an option"]'
                    element = wait.until(EC.visibility_of_element_located((By.XPATH, xpath)))
                    # element.click()
                    actions = ActionChains(driver)
                    actions.move_to_element(element)
                    actions.click()
                    time.sleep(2)
                    text = 'Paylocity Historical Documents'
                    actions.send_keys(text)
                    actions.send_keys(Keys.RETURN)
                    actions.perform()
                    ...
                    element  = driver.find_element(By.XPATH, xpath)
                    element.send_keys("P:\\Test Uploads\\Paystubs\\"+fld_name+"\\"+ufile)
                    time.sleep(2)
                    ...
                    filename = "report_Terminated.txt"
...
def main():
    ...
    emp_flg_start=1
    rows=[]
    select_statement = "SELECT [Employee_ID],[Last_Name],[First_Name],[IsUploaded_Paystubs]"
            ...
            if fld_name in directory_contents:
                if os.path.isdir(path+fld_name):
                    ...
                    select_statement1 = """SELECT [Employee_ID],[Employee_Name],[File_Name],[IsUploaded],[UploadedDateTime]
                                         FROM [HRG].[dbo].[Emp Files Paystubs] WHERE [Employee_ID] = ? ;"""
                    cursor = SQLconnection.cursor()
                ...
                if res == 0:
                    cursor = SQLconnection.cursor()
                    sql_update = """UPDATE [HRG].[dbo].[Emp_Roster] SET [IsUploaded_Paystubs] = ?
                        WHERE  [Employee_ID] =? ;"""
